...
@Component({
  ...
  template: `
    @if (carta()) {
      <div
        class="fixed z-[9999] pointer-events-none shadow-2xl rounded-xl overflow-hidden border border-surface bg-surface-emphasis animate-in fade-in zoom-in duration-200"
        [style.left.px]="x() + 20"
        [style.top.px]="y() - 150"
      >
        <div class="relative w-[250px] h-[350px]">
          <img
            [ngSrc]="obterImagemNormal(carta()!)"
            [alt]="carta()!.name"
            fill

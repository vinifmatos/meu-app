        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div class="md:col-span-4 lg:col-span-3">
            <div class="sticky top-4">
              <div
                class="relative w-full aspect-[2.5/3.5] rounded-xl shadow-2xl overflow-hidden bg-surface-100 dark:bg-surface-800"
              >
                <img
                  [ngSrc]="imagemExibida()"
                  [alt]="carta.name"
            ...
            <div class="flex flex-col gap-4">
              <div class="flex justify-between items-start">
                <div class="flex flex-col gap-1">
                  ...
                  <p class="text-xl font-medium">{{ carta.typeLine }}</p>
                </div>
                @if (carta.manaCost) {
                  <div
                    class="text-2xl font-bold px-4 py-2 rounded-full border border-surface  flex items-center justify-center shrink-0"
                    [innerHTML]="carta.manaCost | simbolos"
                  ></div>
                }
              </div>
              ...
              <p-card>
                <div
                  class="italic whitespace-pre-wrap text-lg"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                <p-card
                  header="Coleção"
                  subheader="{{ carta.set.toUpperCase() }} #{{ carta.collectorNumber }}"
                >
                  <div class="flex items-center gap-2">
                    <p-tag
              ...
              @if (versoes().length > 0) {
                <div class="mt-8">
                  <h3 class="text-2xl font-bold mb-4 uppercase">
                    Artes Disponíveis ({{ idiomaSelecionado().toUpperCase() }})
                  </h3>
                  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    @for (versao of versoes(); track versao.id) {
                      ...
                      >
                        <div
                          class="relative w-full aspect-[2.5/3.5] rounded-md overflow-hidden shadow-sm"
                        >
                          <img
                            [ngSrc]="obterImagemNormal(versao, 'small')"
                            [alt]="versao.name"
                            fill
                            class="object-cover"
                          />
                        ...
                        <div class="flex flex-col items-center">
                          <span class="text-[10px] font-bold uppercase">{{ versao.set }}</span>
                        </div>
                      </div>
                    }
                  </div>
...
export class DetalhesComponent implements OnInit {
  ...
  obterImagemNormal(carta: Carta, size: 'normal' | 'small' = 'normal'): string {
    ...
    if (carta.faces?.length > 0 && carta.faces[0].imageUris?.[size])
      return carta.faces[0].imageUris[size];
    return '';
  }

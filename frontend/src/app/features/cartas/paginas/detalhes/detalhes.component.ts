      ...
      <div class="flex justify-between items-center mb-6">
        ...
        @if (idiomas().length > 1) {
          <div class="flex items-center gap-2">
            <span class="text-sm font-bold text-surface-500">IDIOMA:</span>
            <p-select 
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div class="md:col-span-4 lg:col-span-3">
            <div class="sticky top-4">
              <div class="relative w-full aspect-[2.5/3.5] rounded-xl shadow-2xl overflow-hidden bg-surface-100 dark:bg-surface-800">
                <img 
                  [ngSrc]="obterImagemPrincipal(carta)" 
                  [alt]="carta.name" 
                  fill 
                  priority
                  class="object-cover"
              ...
              @if (carta.faces.length > 1) {
                <div class="grid grid-cols-2 gap-2 mt-4">
                  @for (face of carta.faces; track face.id) {
                    <div class="relative aspect-[2.5/3.5] rounded-md overflow-hidden shadow-md cursor-pointer border-2 border-transparent hover:border-primary-500">
          ...
          <div class="md:col-span-8 lg:col-span-9">
            <div class="flex flex-col gap-4">
              <div class="flex justify-between items-start">
                <div>
                  <h1 class="text-4xl font-bold text-surface-900 dark:text-surface-0 mb-2">{{ carta.name }}</h1>
              ...
              <!-- Lista de Versões -->
              <div class="mt-8">
                <h3 class="text-2xl font-bold mb-4">Outras Impressões ({{ idiomaSelecionado().toUpperCase() }})</h3>
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  @for (versao of versoes(); track versao.id) {
                    <div 
                      (click)="navegarParaVersao(versao.id)"
                      [class.border-primary-500]="versao.id === carta.id"
                      class="flex flex-col items-center gap-2 p-2 border-2 border-transparent rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 cursor-pointer transition-all"
                    >
                      <div class="relative w-full aspect-[2.5/3.5] rounded-md overflow-hidden shadow-sm">
                        <img [ngSrc]="obterImagemPrincipal(versao)" [alt]="versao.name" fill class="object-cover" />
                      </div>
                      <span class="text-xs font-bold uppercase">{{ versao.set }}</span>
                    </div>
                  }
                </div>
              </div>
...
export class DetalhesComponent implements OnInit {
  ...
  carregarDadosCarta(id: number): void {
    this.cartasService.obterCarta(id, this.idiomaSelecionado()).subscribe({
      next: (resposta) => {
        if (resposta.data) {
  ...
  navegarParaVersao(id: number): void {
    this.router.navigate(['/cartas', id]);
  }

  obterImagemPrincipal(carta: Carta): string {
    return carta.imageUris?.normal || carta.faces[0]?.imageUris?.normal || '';
  }

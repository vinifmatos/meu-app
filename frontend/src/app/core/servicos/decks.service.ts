...
export class DecksService {
  ...
  async listarDecks(apenasMeus = false): Promise<Deck[]> {
  ...
  async atualizarCartasDeck(
    deckId: number,
    cartas: { carta_id: number; quantidade: number; eh_comandante: boolean }[],
  ): Promise<Deck | null> {
  ...
  async removerCarta(deckId: number, cartaId: number, removerTudo = false): Promise<Deck | null> {
    const query = removerTudo ? { tudo: 'true' } : {};
    const resposta = await this.api.delete<{ deck: Deck }>(
      `${this.endpoint}/${deckId}/cartas/${cartaId}`,
      { query },
    );
    return resposta.data?.deck ?? null;
  }
  ...
  async validarDeck(id: number): Promise<{ valido: boolean; erros: string[] }> {
    const resposta = await this.api.get<{ valido: boolean; erros: string[] }>(
      `${this.endpoint}/${id}/validar`,
    );
